...
def render_document_selection():
    ...
    st.write(
        "Select the document type you want to verify."
    )
    ...
    st.caption(
        "Passport verification • OCR • MRZ • "
        "document integrity • identity verification"
    )
    ...
    with col1:
        ...
        if st.button(
            ...
            st.info(
                "Visa verification is coming soon."
            )
        ...
        if st.button(
            ...
            st.info(
                "ID card verification is coming soon."
            )
    ...
    with col2:
        ...
        if st.button(
            ...
            st.info(
                "Residence permit verification is coming soon."
            )
        ...
        if st.button(
            ...
            st.info(
                "Travel permit verification is coming soon."
            )
...
def render_sidebar():
    with st.sidebar:
        ...
        with st.expander(
            ...
            st.caption(
                "Developer testing only — does not perform real document analysis."
            )
...
def render_header():
    ...
    st.markdown(
        "#### AI-assisted passport verification and risk assessment"
    )

    st.caption(
        "TRINETRA | SIH 2026 • Problem Statement 26188"
    )
...
<div class="result-card {css_class}">
    <h4 style="margin:0; opacity:0.9;">DOCUMENT RISK ASSESSMENT</h4>
...
def render_verification_pipeline(res: dict):
    ...
    img_qual = (
        "FAIL"
        if res["processing"]["status"] == "INSUFFICIENT_EVIDENCE"
        else "PASS"
    )
    ...
    ocr_stat = (
        f"{res['ocr']['confidence_score'] * 100:.1f}%"
        if res["ocr"]["status"] == "SUCCESS"
        else "FAIL"
    )

    mrz_stat = (
        f"{res['mrz']['checksums_passed']} / "
        f"{res['mrz']['total_checksums']}"
        if res["mrz"]["status"] == "SUCCESS"
        else "FAIL"
    )

    tamp_stat = (
        "HIGH"
        if res["tampering"]["tampering_detected"]
        else (
            "FAIL"
            if res["tampering"]["status"] != "SUCCESS"
            else "LOW"
        )
    )
    ...
    if face["status"] == "SUCCESS":
        ...
        id_stat = "NOT VERIFIED"
    else:
        id_stat = "FAILED"

    steps = [
        ("IMAGE QUALITY", img_qual),
        ("OCR", ocr_stat),
        ("MRZ", mrz_stat),
        ("VALIDATION", doc_val),
        ("TAMPERING", tamp_stat),
...
def render_document_information(doc: dict):
    ...
    with st.container(border=True):
        ...
        c1.caption("Document Type")
        c1.write(
            doc.get("document_type")
        )

        c2.caption("Issuing Country")
        c2.write(
            doc.get("issuing_country")
        )

        c3.caption("Document Number")
        c3.write(
        ...
        c5.caption("Given Names")
        c5.write(
            doc.get("given_names")
        )

        c6.caption("Date of Birth")
        c6.write(
            doc.get("date_of_birth")
            or "Not available"
        )

        c7.caption("Expiry Date")
        c7.write(
            doc.get("expiry_date")
            or "Not available"
        )
    ...
    with c1:
        ...
        with st.container(border=True):
            ...
            if ocr.get("status") == "SUCCESS":
                ...
                st.write(
                    f"Confidence: **"
                    f"{ocr.get('confidence_score', 0) * 100:.1f}%**"
                )
    ...
    with c2:
        ...
        with st.container(border=True):
            ...
            if mrz.get("status") == "SUCCESS":
                ...
                st.write(
                    f"Checksums Passed: **"
                    f"{mrz.get('checksums_passed')} / "
                    f"{mrz.get('total_checksums')}**"
                )

                st.write(
                    f"MRZ Valid: **"
                    f"{'Yes' if mrz.get('is_valid') else 'No'}**"
                )

                st.write(
                    f"DOB Match: **"
                    f"{'Yes' if mrz.get('dob_match') else 'No'}**"
                )

                st.write(
                    f"Expiry Match: **"
                    f"{'Yes' if mrz.get('expiry_match') else 'No'}**"
                )
    ...
    with st.container(border=True):
        ...
        if tamp.get("status") == "SUCCESS":
            ...
            if not tamp.get("regions"):
                ...
                st.info(
                    "No suspicious regions reported."
                )

            else:

                st.warning(
                    "Regions highlighted on image "
                    "(Integration Pending)"
                )
...
def render_identity_verification(face: dict):
    ...
    with st.container(border=True):
        ...
        if status == "SUCCESS":
            ...
            st.info(
                "No presented-person image was provided. "
                "Document screening was completed, but "
            ...
            st.info(
                "The document was processed, but a reliable "
                "passport portrait could not be used for "
...
def render_screening_information(
    processing: dict
):
    ...
    with st.container(border=True):
        ...
        st.write(
            f"**Processing Time:** "
            f"{processing.get('processing_time')}s"
        )

        st.write(
            f"**Pipeline Version:** "
            f"{processing.get('pipeline_version')}"
        )
...
def main():
    ...
    passport_file, face_file, analyze_btn = (
    ...
    if analyze_btn:
        ...
        if not passport_file:
            ...
            st.error(
                "Operation Blocked: Passport document "
        ...
        try:
            ...
            with st.spinner(
                "Processing document through API..."
            ):
            ...
            if response.status_code == 200:
                ...
                render_document_integrity(
                    result["ocr"],
                    result["mrz"]
                )

                render_tampering_analysis(
                    result["tampering"]
                )
                ...
                render_evidence_and_analysis(
                    result["risk_assessment"]
                )

                render_officer_review(
                    result["risk_assessment"]["recommendation"]
                )

                render_screening_information(
                    result["processing"]
                )
            ...
            st.error(
                "API Unavailable: Cannot connect to the "
                "TRINETRA backend. Ensure FastAPI is running."
            )

        except Exception:

            st.error(
                "System Fault: An unexpected UI error occurred."
            )
